...
**Note** if widgets are created for distribution, `widgetloader.php` must be included, so for that purpose it is best to place the file in the plugin package for the purpose of updating accurately, then load it via the plugin pilot file. ![See The Plugin Development Guide](https://github.com/WebsiteDons/elementor-widget-creator-library#creating-a-distributable-plugin)
...
![See In Depth Details](/WebsiteDons/elementor-widget-creator-library/tree/main/elementor-widgets/widgets/image#xml-attributes)
