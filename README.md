...
With `WP_DEBUG` enabled, all controls generated by the xml markup and passed through the `foreach()` loop returns an error that the element name is being redeclared
...
Elementor method `add_control_to_stack()`
